import * as rfpRepository from "../repositories/rfpRepository";
import * as supplierRepository from "../repositories/supplierRepository";
import { TokenPayload } from "../utils/tokens";

export const supplierListingData = async (options: any, auth: TokenPayload) => {
  const sort = "createdAt";
  const filter: {
    orderBy: object;
    where: Record<string, unknown>;
    select: object;
    take?: number;
    skip?: number;
  } = {
    orderBy: {
      [options.sort || sort]: options.order,
    },
    where: {},
    select: {
      id: true,
      name: true,
      email: true,
      code: true,
      createdAt: true,
      searchString: true,
      creator: {
        select: {
          id: true,
          name: true,
        },
      },
    },
  };

  if (options.search) {
    filter.where["searchString"] = {
      contains: options.search,
      mode: "insensitive",
    };
  }
  if (options.creatorId) {
    filter.where["creatorId"] = options.creatorId;
  }
  if (options.active !== undefined) {
    filter.where["active"] = options.active;
  }
  if (options.isRegistered !== undefined) {
    filter.where["isRegistered"] = options.isRegistered;
  }
  if (options.status) {
    filter.where["status"] = options.status;
  }
  if (options.take) filter.take = options.take;
  if (options.skip) filter.skip = options.skip;

  const [suppliers = [], count = 0] = await Promise.all([
    supplierRepository.findManyForListing(filter),
    supplierRepository.countForListing(filter.where),
  ]);

  return { suppliers, count };
};

export const rfpForListing = async (options: any, auth: TokenPayload) => {
  const filter: {
    orderBy: object;
    where: Record<string, unknown>;
    select: object;
    include: object;
    take?: number;
    skip?: number;
  } = {
    orderBy: {
      [options.sort || "createdAt"]: options.order || "desc",
    },
    where: {
      userId: auth.userId,
    },
    select: {
      id: true,
      code: true,
      status: true,
      subject: true,
      createdAt: true,
      userId: true,
      user: {
        select: {
          id: true,
          name: true,
        },
      },
    },
    include: {
      suppliers: {
        select: {
          supplierId: true,
          supplier: {
            select: {
              id: true,
              email: true,
              name: true,
            },
          },
        },
      },
    },
  };

  if (options.search) {
    filter.where["searchString"] = {
      contains: options.search,
      mode: "insensitive",
    };
  }
  if (options.status) {
    filter.where["status"] = options.status;
  }
  if (options.active !== undefined) {
    filter.where["active"] = options.active;
  }
  if (options.supplierId) {
    filter.where["suppliers"] = {
      some: {
        supplierId: options.supplierId,
      },
    };
  }
  if (options.take) filter.take = options.take;
  if (options.skip) filter.skip = options.skip;

  const [rfps = [], count] = await Promise.all([
    rfpRepository.findManyForListing(filter),
    rfpRepository.countForListing(filter.where),
  ]);
  return { rfps, count };
};
